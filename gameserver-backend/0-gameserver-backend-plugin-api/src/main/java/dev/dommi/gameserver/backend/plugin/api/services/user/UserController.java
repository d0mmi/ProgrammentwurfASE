...
public class UserController {
    ...
    public void getOne(Context ctx) {
        User user = userBridge.findById(validPathParamUserId(ctx));
        if (user == null) {
    ...
    public void getLoggedIn(Context ctx) {
        ...
        if (userId > -1) {
            User user = userBridge.findById(userId);
            if (user == null) {
                throw new NotFoundResponse(NOT_FOUND_RESPONSE);
            } else {
                ctx.json(user);
    ...
    public void update(Context ctx) {
        ...
        if (validUserRequest(ctx)) {
            ...
            if (user == null) {
                ...
                if (userBridge.update(user.id, newUser.name, newUser.email)) {
                    ctx.status(204);
                } else {
    ...
    public void changePassword(Context ctx) {
        ...
        if (userId > -1) {
            ...
            if (userBridge.changePassword(userId, request.oldpw, request.newpw)) {
                ctx.status(204);
            } else {
                throw new BadRequestResponse();
            }
